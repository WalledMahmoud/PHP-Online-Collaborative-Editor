var FirepadUserList = (function() {
  function FirepadUserList(ref, place, userId, displayName) {
    ...
    this.displayName_ = displayName || 'Programmer ' + Math.floor(Math.random() * 1000);
  ...
  FirepadUserList.prototype.makeUserEntryForSelf_ = function() {
    ...
    var nameHint = elt('div', "What's Your Name ?", { 'class': 'firepad-userlist-name-hint typing'} );
    ...
    on(nameInput, 'change', function(e) {
      var name = nameInput.value || "Programmer " + Math.floor(Math.random() * 1000);
  ...
  FirepadUserList.prototype.makeUserEntriesForOthers_ = function() {
    ...
    function updateChild(userSnapshot, prevChildName) {
      ...
      if (typeof name !== 'string') { name = 'Programmer'; }
      ...
      var colorDiv = elt('div', null, { 'class': 'firepad-userlist-color-indicator open-blink' });
      colorDiv.style.backgroundColor = color;

      var nameDiv = elt('div', name || 'Programmer', { 'class': 'firepad-userlist-name' });
